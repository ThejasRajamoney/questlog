...
export function AssignmentGrader() {
  ...
  const gradeAssignment = async () => {
    ...
    const PROMPT = `You are an expert academic assignment grader. Carefully analyze this assignment image and respond ONLY with a valid JSON object in this exact format (no markdown, no explanation outside JSON):
{
  "stars": <integer 1-5>,
...
2 stars = Below average, significant gaps
3 stars = Average, acceptable but room to grow
      ...
      <div className="p-5 space-y-4">
        ...
        {!imagePreview ? (
          ...
          <div className="relative rounded-2xl overflow-hidden bg-gray-50 border border-gray-100">
            ...
            <button
              onClick={clearImage}
              className="absolute top-2 right-2 w-8 h-8 bg-gray-900/60 hover:bg-rose-500 text-white rounded-full flex items-center justify-center transition-colors"
            >
              <X size={16} />
            </button>
          </div>
          ...
          <button
            ...
            className={clsx(
              ...
              loading
                ...
                : 'bg-gradient-to-r from-violet-500 to-fuchsia-500 hover:opacity-90 active:scale-95 shadow-md'
            )}
          >
            {loading ? (
              <>
          ...
          <div className="space-y-4 slide-up">
            ...
            {result.improvements?.length > 0 && (
              <div className="bg-amber-50 rounded-2xl p-4 space-y-2">
                <h4 className="text-xs font-black text-amber-700 uppercase tracking-wide">🔧 How to Improve</h4>
                <ul className="space-y-1.5">
            ...
            <button
              onClick={clearImage}
              className="w-full py-3 rounded-2xl font-bold text-gray-500 text-sm border-2 border-dashed border-gray-200 hover:border-violet-300 hover:text-violet-500 transition-colors"
            >
